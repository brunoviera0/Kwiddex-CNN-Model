...
# Training parameters
DOC_BACKBONE_PATH = os.environ.get("DOC_BACKBONE_PATH")
...
def main():
    if not DOC_BACKBONE_PATH or not os.path.isfile(DOC_BACKBONE_PATH):
        raise FileNotFoundError(
            f"DOC_BACKBONE_PATH is not set or file not found: {DOC_BACKBONE_PATH}. "
            f"Set it to last model from Part 1"
        )
    
    model = models.resnet18(weights=None)

    # Load document-pretrained backbone from Part 1
    print(f"Loading document backbone from: {DOC_BACKBONE_PATH}")
    state = torch.load(DOC_BACKBONE_PATH, map_location=DEVICE)
    model.load_state_dict(state, strict=False)

    model.fc = nn.Linear(model.fc.in_features, len(classes)) # make sure that classes are "real" and "fake"
...
if __name__ == "__main__":
    ...
    # export DOC_BACKBONE_PATH="last_dataset.pt" from Part 1
