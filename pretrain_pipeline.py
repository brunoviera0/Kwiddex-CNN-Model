...
def looks_like_imagefolder_root(d):
...
def find_dataset_root(extract_dir):
    for root, dirs, files in os.walk(extract_dir):
        if looks_like_imagefolder_root(root):
            print("Detected dataset root:", root)
            return root

    print("No structured root found. Using extract dir.")
    return extract_dir
...
def split_dataset(base_dir):
    ...
    for cls in classes:
        ...
        for split, subset in splits.items():
            result[split][cls] = [
                os.path.join(src, f) for f in subset
            ]
...
def write_manifest(split_map, dataset_name):
    ...
    for split, classes in split_map.items():
        manifest[split] = {
            cls: len(files) for cls, files in classes.items()
        }

    blob = bucket.blob(f"{dataset_name}/manifest.json")
...
def main():
    ...
    parser.add_argument("--dataset", required=True, help="Dataset name in bucket")
